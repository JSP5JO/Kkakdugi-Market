...
insert into gallery (idx, fileName)
	values((select idx from writing order by idx desc limit 1), '100_나이키조던신발');
-- 글 삽입
insert into writing (categoryIdx, userId, subject, content, readCount)
	values('5', 'iU123', '[구매]맥북 프로 파시는 분?', '12인치 중고 사고 싶은데 파실분 없나요?', 5);
-- 갤러리 사진 삽입
insert into gallery (idx, fileName)
	values((select idx from writing order by idx desc limit 1), '101_맥북12인치');
-- 글 삽입
insert into writing (categoryIdx, userId, subject, content)
	values('2', 'jj3458', '[판매]서핑 보드 팔아요', '5만원에 팝니다 직거래는 서울에서만 할래요.');
-- 갤러리 사진 삽입
insert into gallery (idx, fileName)
	values((select idx from writing order by idx desc limit 1), '102_서핑보드');
-- 글 삽입
insert into writing (categoryIdx, userId, subject, content)
	values('3', 'uhj9494', '[시세/질문] 통기타 가격 질문드려요.', '10만원 선에서 살만한 기타 추천해주세요.');
-- 갤러리 사진 삽입
insert into gallery (idx, fileName)
	values((select idx from writing order by idx desc limit 1), '103_통기타');
-- 글 삽입
...
-- 갤러리 사진 삽입
insert into gallery (idx, fileName)
	values((select idx from writing order by idx desc limit 1), '104_침대');
-- 댓글 삽입
...
-- 갤러리 사진 삽입
insert into gallery (idx, fileName)
	values((select idx from writing order by idx desc limit 1), '105_명품화장품세트');
